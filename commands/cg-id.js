    ...
    async execute(interaction) {
        try {
            ...
            db.get("SELECT * FROM users WHERE user_id = ?", [user.id], async (err, row) => {
                if (err) return safeReply(interaction, "❌ Error al obtener los datos de usuario.");
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: "⏱️ XP Voz", value: `${voiceXP}`, inline: true },
                        { name: "📊 XP Mensajes", value: `${xp}`, inline: true },
            ...
            await safeReply(interaction, "❌ Ocurrió un error al mostrar la información.");
        }
    }
};
